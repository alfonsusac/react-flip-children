...
function AppsExample() {
  ...
  const [arr, setArr] = useState(Array.from({ length: 20 }, (_, i) => idCountRef.current++))
  ...
  return (
    <>
      ...
      <div className="rounded-md grid grid-cols-5 sm:grid-cols-6 gap-4 sm:gap-6">
        <ReorderArray>
          {arr.map((item, index) => (
            <div key={item} className="text-center relative group animate-appear"
              style={{ zIndex: item }}
            >
              <div onClick={() => remove(item)} className="absolute -top-2 -right-2 w-6 h-6 rounded-full bg-white z-20 flex items-center justify-center transition-all opacity-0 group-hover:opacity-100 cursor-pointer">
                <MaterialSymbolsAddRounded className="text-black/50 rotate-45" />
              </div>
              <div className="rounded-xl sm:rounded-2xl flex items-center justify-center p-3 bg-[#6cab88] font-extrabold text-8xl leading-[0] grow aspect-square w-full overflow-hidden relative animate-[spasm_1s_linear_infinite]"
                style={{ backgroundColor: colors2[item % colors2.length], animationDelay: `${ -item * 200 }ms` }}
              >
                {apps[item % apps.length].icon({
                  className: "text-white w-20 h-20 relative z-20",
                })}
                <div className="w-8 h-8 bg-black/20 absolute rounded-full blur-xl" />
              </div>
              <div className="text-[#4f4d31aa] text-sm mt-1 font-bold">{apps[item % apps.length].name}</div>
            </div>
          ))}
