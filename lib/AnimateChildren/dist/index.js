...
function cloneWithMergedRef(element, ref, props) {
  return (0, import_react.cloneElement)(element, __spreadValues({ ref }, props));
}
  ...
  (0, import_react2.useEffect)(() => {
    ...
    const newRender = flatMap(
      ...
      (child) => {
        ...
        (_a = entry.ref.current) == null ? void 0 : _a.removeAttribute("data-deleting");
    ...
    rendered.forEach(
      (child, index) => {
        ...
        if (!(entry == null ? void 0 : entry.ref.current)) return;
        const node = entry.ref.current;
        (_b = parent.node) != null ? _b : parent.node = (_a = node.parentElement) != null ? _a : parent.node;
  ...
  (0, import_react2.useLayoutEffect)(() => {
    ...
    rendered.forEach(
      (child) => {
        ...
        if (!(entry == null ? void 0 : entry.ref.current)) return;
        const node = entry.ref.current;
        node.getAnimations().filter(isCSSAnimation).forEach((animation, index) => {
          var _a, _b;
          return animation.currentTime = (_b = (_a = entry.cssAnimationTimes) == null ? void 0 : _a[index]) != null ? _b : 0;
        });
    ...
    requestAnimationFrame(() => {
      ...
      data.forEach((entry) => {
        var _a;
        return (_a = entry.ref.current) == null ? void 0 : _a.removeAttribute("data-adding");
      });
    });
  }, [rendered]);
...
function useParent() {
  const parent = useRefObject({
    ...
    node: void 0,
    saveNode(node) {
      parent.node = node != null ? node : parent.node;
    },
    saveRect() {
...
function useRefObject(defaultObj) {
  const ref = (0, import_react2.useRef)(defaultObj != null ? defaultObj : {});
  return ref.current;
}
